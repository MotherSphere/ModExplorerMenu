...
namespace Hooks
{
	...
	void Install()
	{
		// AllocTrampoline FIXE la taille demandee, il ne l'additionne pas, et
		// InitTrampoline est garde par un std::call_once : seul le PREMIER
		// appel cree reellement le tampon, les suivants sont sans effet. Les
		// quatre appels d'origine ne donnaient donc que 14 octets en tout, et
		// le second hook echouait sur "Failed to handle allocation request".
		// Une seule allocation, dimensionnee pour l'ensemble : 14 + 8 + 14 + 14
		// (write_5branch consomme 14 octets, write_6branch 8), arrondi a 64.
